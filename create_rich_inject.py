"""Batch-create rich shrine articles via direct clipboard injection.

Instead of copying fragments from Enoshima and swapping QIDs, this script
directly injects the clipboard template into localStorage with the correct
deity QID already set. This skips the Enoshima visit and swap step entirely.

Usage:
    python create_rich_inject.py                        # Dry run
    python create_rich_inject.py --apply                # Create articles
    python create_rich_inject.py --apply --max-edits 5  # Limit to 5
    python create_rich_inject.py --apply --headed       # See the browser
"""
...
# The clipboard template extracted from Enoshima Shrine
# Item 0 = deity fragment, Item 1 = location fragment
CLIPBOARD_TEMPLATE = [
    {
        "itemId": "Q8776414.2#1",
...
def build_clipboard(deity_qid):
    """Build clipboard data with the deity QID injected directly."""
...
def inject_clipboard(page, clipboard_data):
    """Inject clipboard into both localStorage and the Pinia store."""
    page.evaluate("""(data) => {
        // Write to localStorage
        localStorage.setItem('ext-wikilambda-app-clipboard', JSON.stringify(data));

        // Write to Pinia store
...
def create_article_inject(page, shrine_qid, deity_qid):
    """Create a rich article by injecting clipboard directly. Returns 'created' or 'error'."""
    print(f"\n{'='*50}", flush=True)
    ...
    print(f"  Pass 1: injecting clipboard + pasting location...", flush=True)
    ...
    # === Pass 2: Edit to add deity fragment ===
    print(f"  Pass 2: injecting clipboard + pasting deity...", flush=True)
    page.goto(f"{WIKI_URL}/w/index.php?title={shrine_qid}&action=edit")
    page.wait_for_load_state("networkidle")
    time.sleep(6)

    # Re-inject since navigation reloads localStorage
    inject_clipboard(page, clipboard)
    time.sleep(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create rich shrine articles via direct clipboard injection")
    ...
    with sync_playwright() as p:
        ...
        for i, s in enumerate(todo):
            if stats["created"] >= args.max_edits:
                break
            try:
                result = create_article_inject(page, s["shrine"], s["deity"])
