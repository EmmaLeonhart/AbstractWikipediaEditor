...
- Multiple ``{{...}}`` calls within a paragraph bundle into one
  Z33068([Z1, sentence1, sentence2, ...], $lang) clipboard item
...
class TestBundledParagraphCompilation:
    ...
    def test_k2_is_lang_arg_ref(self):
        """Z33068K2 must be a $lang Z18 ref so the function can pick the
        right between-sentence separator (e.g. no separator in CJK)."""
        result = compile_template(JUPITER_WIKITEXT, {"subject": "Q319"})
        assert result[0]["value"]["Z33068K2"]["Z18K1"]["Z6K1"] == "Z825K2"

    def test_typed_list_holds_all_calls_no_separators(self):
        """The bundled list is [Z1, call1, call2, ...] — no separator
        strings; Z33068 emits the right separator itself based on K2."""
        result = compile_template(JUPITER_WIKITEXT, {"subject": "Q319"})
